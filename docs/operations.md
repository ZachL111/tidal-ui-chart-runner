...
`tidal-ui-chart-runner` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for chart scenarios with deny and allow fixtures, explainable decision traces, and synthetic fixtures only.
...
The score is deterministic. The current threshold is 154. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 6.
